...
for(compartment in unique(preds2_fd$svm.pred)){  # make plots
  ...
  hist(rowMeans(data.frame(lDist_NA[which(names(lDist_NA) %in% genesC)])), breaks = 500, 
...
# lCompartmentKrusk = list()
# for(compartment in unique(preds2_fd$svm.pred)){  # compare statistically
#   genesC = preds2_fd[which(preds2_fd$svm.pred == compartment),'Accession']
#   genesU = preds2_fd[which(preds2_fd$svm.pred == "unknown"),'Accession']
#    tmpDF = cbind(data.frame(rowMeans(data.frame(lDist_NA[which(names(lDist_NA) %in% genesU)]))),
#                  data.frame(rowMeans(data.frame(lDist_NA[which(names(lDist_NA) %in% genesC)]))))
#    colnames(tmpDF) = c("U", "C")
#    lCompartmentKrusk[[compartment]] = kruskal.test(U ~ C, data = tmpDF)
#    
# }
...
group.colors_y = c("Mitochondrion" = "deeppink", "Proteasome regulatory subunit" = "aquamarine3", 
                   "Ribosome" = "darkgoldenrod","unknown" = "white", "Nuclei" = "blue4",
